...
void Overlay::paintEvent(QPaintEvent *e)
{
    ...
    colorC.getHsv(&h,&s,&v);
    colorC.setHsv(h<1?359:--h,s,v);

    colorD.getHsv(&h,&s,&v);
    colorD.setHsv(h>359?0:++h,s,v);
    ...
    p.setPen(colorD);
    p.drawEllipse(ammo,5,5 );
    p.drawLine(ammo.rx()-9,ammo.ry(),ammo.rx()+11,ammo.ry());
    p.drawLine(ammo.rx(),ammo.ry()-9,ammo.rx(),ammo.ry()+11);
